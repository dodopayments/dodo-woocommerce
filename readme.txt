...
* Feature: send customers who abandon the hosted checkout back to your store. Defaults to the order pay page so they can retry; previously no cancel URL was sent at all.
...
* Feature: send the order's billing phone number and company to the hosted checkout so customers are not asked for them twice. Both are on by default and can be switched off.
